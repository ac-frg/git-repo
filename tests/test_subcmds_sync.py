...
class KeyboardInterruptTest(unittest.TestCase):
    ...
    @mock.patch("subcmds.sync.multiprocessing.current_process")
    def test_fetch_one_keyboard_interrupt_main_process(
        self, mock_current_process
    ):
        """Test that _FetchOne re-raises KeyboardInterrupt in MainProcess."""
    ...
    @mock.patch("subcmds.sync.multiprocessing.current_process")
    def test_fetch_one_keyboard_interrupt_worker_process(
        self, mock_current_process
    ):
        """Test that _FetchOne suppresses KeyboardInterrupt in worker process."""
        mock_current_process.return_value.name = "ForkPoolWorker-1"
        self.project.Sync_NetworkHalf.side_effect = KeyboardInterrupt()
