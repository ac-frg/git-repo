...
class SparseCheckoutTests(unittest.TestCase):
    ...
    def test_configure_sparse_checkout_calls_git(self):
        """Test that sparse-checkout calls git with correct arguments."""
        with TempGitTree() as tempdir:
            fakeproj = FakeProject(tempdir)
            ...
            with mock.patch.object(
                fakeproj.work_git, "sparse_checkout"
            ) as mock_sparse:
                fakeproj._ConfigureSparseCheckout(["path1", "path2"])

                self.assertEqual(mock_sparse.call_count, 2)
                mock_sparse.assert_any_call("init", "--cone")
                mock_sparse.assert_any_call("set", "path1", "path2")


class SparseCheckoutExistingRepoTest(unittest.TestCase):
    """Test sparse-checkout works on repos with a full clone."""
    ...
    def test_sparse_checkout_on_existing_full_clone(self):
        """Test applying sparse-checkout to existing repo with full clone."""
        # Skip if git version is too old
        if not git_command.git_require((2, 25, 0)):
            self.skipTest("Git 2.25.0+ required for sparse-checkout")

        # Verify we have a full clone initially
        main_app = os.path.join(self.existing_clone, "src/main/app.py")
        test_app = os.path.join(self.existing_clone, "src/tests/test_app.py")
        guide = os.path.join(self.existing_clone, "docs/guide.md")
        makefile = os.path.join(self.existing_clone, "build/Makefile")

        self.assertTrue(os.path.exists(main_app))
        self.assertTrue(os.path.exists(test_app))
        ...
        # These should not exist in working tree (but still in git history)
        self.assertFalse(
            os.path.exists(test_app),
    ...
    def test_sparse_checkout_with_updates(self):
        """Test sparse-checkout only fetches updates for configured paths."""
        # Skip if git version is too old
        if not git_command.git_require((2, 25, 0)):
            self.skipTest("Git 2.25.0+ required for sparse-checkout")
        ...
        # Create a new commit in upstream that modifies files in both
        # sparse and non-sparse paths
        ...
        app_path = os.path.join(self.existing_clone, "src/main/app.py")
        with open(app_path) as f:
            content = f.read()
            self.assertIn(
                "Updated in sparse path",
                content,
                "File in sparse path should be updated",
            )

        # Verify the non-sparse path file is still not in working tree
        makefile_path = os.path.join(self.existing_clone, "build/Makefile")
        self.assertFalse(
