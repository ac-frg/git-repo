...
Requires Git 2.25.0 or later, which is when `git sparse-checkout` and cone mode
were introduced. Declaring a sparse-path with an older Git fails the sync for
that project.

Declaring at least one sparse-path is what enables Git sparse-checkout for
the project: the listed paths are checked out, and other directories are left
out of the working directory. Cone mode also checks out the files sitting
directly in the project's top-level directory and in each leading directory of
a sparse-path, so `src/backend` brings along the top-level files and the files
directly under `src/` too. A project with no sparse-path elements is checked
out in full.
The checkout uses [cone mode](https://git-scm.com/docs/git-sparse-checkout#_internalscone_mode_handling)
...
`?`, `[`, `]`, and `\` are not glob syntax in a `path`. In fact, Git 2.36.0 and
later reject them with `specify directories rather than patterns`, which
surfaces as a sync failure for the project. Earlier versions starting with
2.25.0 have no such check and are treated as literals.

Sparse-checkout is configured only when the project's working directory is
first created. Changes to `sparse-path` in the manifest (added, changed, or
removed) are not applied to a project that is already checked out. To change
the paths in an existing checkout, run `git sparse-checkout` in the project
directly, or remove the project's working directory and sync again.

Because sparse-checkout only prunes the working directory, a project with at
least one sparse-path is fetched with `--filter=blob:none` so that blobs which
will never be checked out are not downloaded. The filter from
`repo init --partial-clone --clone-filter` takes precedence, and projects listed
in `repo init --partial-clone-exclude` are fetched in full as usual.
