...
If the repo command that triggered the hook supports a "yes" option (e.g., `repo upload --yes`), this option is propagated to the hook's `main` function as `yes` parameter (defaulting to `False`). Hooks can use this to bypass interactive confirmation prompts when they can automatically fix issues.
...
def main(project_list, worktree_list=None, yes=False, **kwargs):
    ...
    Args:
      ...
      yes: Whether to answer yes to all safe prompts (see [Safe Prompts](#safe-prompts)).
